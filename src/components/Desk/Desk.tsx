...
export function Desk({ tasks, columns }: DeskProps) {
  ...
  useEffect(() => {
    ...
    const hoveredCol = (Object.keys(colCoords) as Status[]).find((name) => {
      const distanceX =
        colCoords[name]?.left - draggedDX - positions[draggedId].left;
      return Math.abs(distanceX) < colCoords[name]?.width * INTERSECTION_RATIO;
    });

    const foundHoveredTask = Object.keys(positions)
      ...
      .find((el) => {
        ...
        return (
          Math.abs(draggedPosition - keyPosition) <
          positions[+el].height * INTERSECTION_RATIO
        );
      });
  ...
  const dropHandler = () => {
    const isInserted = !!Object.values(positions).find(
      (el) => el.index > positions[draggedId].index,
    );
    setConfig((prevState) =>
      isInserted
        ? updateConfigInsert(prevState, positions)
        : updateConfigAdd(prevState, draggedId, hoveredColumn),
    );
  };
